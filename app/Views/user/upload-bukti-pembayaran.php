...
<div class="container-xxl position-absolute top-50 start-50 translate-middle">
    <div class="text-center text-white mb-5">
        ...
        <h5 class="footer">Halaman Upload Bukti Pembayaran</h5>
    </div>
    <div>
        <form action="/order/uploadPembayaran/<?= $order['order_id']; ?>" method="POST" enctype="multipart/form-data">
            <div class="mb-5 mx-auto col-4 text-center">
                <label for="formFile" class="form-label footer text-white">Silahkan upload bukti pembayaran sebesar Rp. <?= number_format($order['total_price'], 2, ",", "."); ?></label>
                <input class="form-control" type="file" id="formFile" name="image_payment" required>
                <button type="submit" name="submit" class="mt-3 bg-yellow text-white shadow-lg border-0 py-1 px-5 rounded-3" onclick="return confirm('Upload bukti transfer?')">Upload</button>
            </div>
        </form>
    </div>
